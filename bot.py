...
class Bot:
    ...
    def select_target_meteor(self, meteors: list[Meteor], game_message: GameMessage) -> Meteor:
        ...
        medium_meteors: list[Meteor] = [meteor for meteor in candidate_meteors if meteor.meteorType == MeteorType.Medium]
        ...
        if self.target_queue and not small_meteors:
            ...
            sorted_candidates: list[Meteor] = [meteor for _, meteor in sorted(zip(scores, candidate_meteors), reverse=True)]
    ...
    def score_meteors(self, meteors: list[Meteor], game_message: GameMessage) -> list[float]:
        ...
        for meteor in meteors:
            dist_from_cannon_x: float = abs(meteor.position.x - game_message.cannon.position.x)
            dist_from_center_y: float = abs(meteor.position.y - game_message.constants.world.height / 2)
            scores.append(2/dist_from_cannon_x + 1/dist_from_center_y)
        return scores
    ...
    def update_pending_collisions(self, game_message: GameMessage) -> None:
        for rocket in game_message.rockets:
            for meteor in game_message.meteors:
                ...
                if will_collide and not any((collision.rocket.id == rocket.id and collision.meteor.id == meteor.id) for collision in self.pending_collisions):
    ...
    def will_collide(self, rocket: Projectile, meteor: Meteor) -> bool:
        ...
        mindist = sqrt((mintime*(r_vel_x - m_vel_x) + r_pos_x - m_pos_x)**2 + (mintime*(r_vel_y - m_vel_y) + r_pos_y - m_pos_y)**2)  
    ...
    def remove_duplicate_collisions(self) -> None:
        ...
        self.pending_collisions = [collision for i, collision in enumerate(self.pending_collisions)
                                    if not any((collision.rocket.id == other.rocket.id or collision.meteor.id == other.meteor.id)
    ...
    def update_shot_rockets(self, game_message: GameMessage) -> None:
        for shot in self.shot_rockets:
            ...
            elif shot.target.id == -1 and shot.rocket_id in [collision.rocket.id for collision in self.actual_collisions]:
                shot.target.id = [collision.meteor.id for collision in self.actual_collisions if collision.rocket.id == shot.rocket_id][0]
    ...
    def print_missed_shots(self) -> None:
        ...
        for shot in missed_shots:
            if shot.rocket_id in [collision.rocket.id for collision in self.actual_collisions]:
                actual_hit = [collision.meteor.id for collision in self.actual_collisions if collision.rocket.id == shot.rocket_id][0]
                print(f"[{shot.time}] Rocket {shot.rocket_id} aimed at meteor {shot.target.id} of type {shot.target.meteorType}, but hit {actual_hit} instead. ({shot.reason})")
            else:
                print(f"[{shot.time}] Rocket {shot.rocket_id} aimed at meteor {shot.target.id} of type {shot.target.meteorType}, but simply missed. ({shot.reason})")